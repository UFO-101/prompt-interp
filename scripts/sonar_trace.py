"""Show detailed trace of decoded prompts during optimization."""
...
for st in range(40):
    ...
    with torch.no_grad():
        ...
        z_drift = (z - zi).norm().item()

    # Show every step
    print(f"\nStep {st}: loss={l.item():.2f}, acc={c}/6, z_drift={z_drift:.4f}", flush=True)
    print(f"  Decoded: '{decoded}'", flush=True)
    print(f"  Predictions:", flush=True)
    for inp, tgt, pred, correct in er:
        sym = '✓' if correct else '✗'
