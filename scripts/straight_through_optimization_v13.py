"""
V13: Optimized version with:
- Early stopping when diverging (loss > 2x initial or z0_acc drops to 0 for 3 steps)
- Multiple seeds tested efficiently
- Better hyperparameter search
- Track and save the best results
"""
...
def optimize_single_run(seed_text, examples, lr=0.001, max_steps=30, grad_clip=0.5, verbose=True):
    ...
    for step in range(max_steps):
        ...
        z0_acc = evaluate_with_z0(tokens, examples)[0]
...
# ============================================================================
print("\n" + "=" * 70)
print("V13: Hyperparameter search with early stopping")
print("=" * 70)
...
grad_clips = [0.3, 0.5, 1.0]

all_results = []

# Quick search: try each seed with default params first
print("\n--- Phase 1: Quick seed screening ---")
for seed in seeds:
    print(f"\nSeed: '{seed[:40]}...'")
    result = optimize_single_run(seed, examples, lr=0.001, max_steps=20, verbose=True)
    all_results.append(result)
    print(f"  >>> Best: {result['best_acc']:.0%} at step {result['best_step']}")

# Find best seeds
best_seeds = sorted(all_results, key=lambda x: -x['best_acc'])[:3]
print(f"\n--- Top seeds: {[s['seed'][:30] for s in best_seeds]} ---")

# Hyperparameter search on best seeds
print("\n--- Phase 2: Hyperparameter search on top seeds ---")
for seed_result in best_seeds:
    seed = seed_result['seed']
    print(f"\nSeed: '{seed[:40]}...'")

    for lr in lrs:
        for grad_clip in grad_clips:
            print(f"  lr={lr}, clip={grad_clip}:")
            result = optimize_single_run(seed, examples, lr=lr, max_steps=25,
                                        grad_clip=grad_clip, verbose=False)
            all_results.append(result)
            print(f"    Best: {result['best_acc']:.0%} at step {result['best_step']} -> '{result['best_prompt'][:35]}...'")
...
for r in all_results_sorted[:10]:
    if r['best_prompt'] not in seen_prompts:
        seen_prompts.add(r['best_prompt'])
        print(f"{r['best_acc']:.0%} (step {r['best_step']}, lr={r['lr']}, clip={r['grad_clip']})")
        print(f"  Seed: '{r['seed'][:40]}...'")
...
print(f"Config: lr={best['lr']}, grad_clip={best['grad_clip']}")
print(f"{'=' * 70}")
