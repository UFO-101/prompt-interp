#!/usr/bin/env python3
"""
Improved EPO with ContextBench enhancements:
...
Based on "ContextBench: Modifying Contexts for Targeted Latent Activation"
"""
import torch
...
class ImprovedEPO:
    """EPO with Assist and Inpainting improvements from ContextBench."""
    ...
    def __init__(
        ...
        # EPO-Assist settings (from paper: every n=50 iterations)
        assist_interval: int = 50,
        # EPO-Inpainting settings (from paper: every n=15 iterations, top 25%)
        inpaint_interval: int = 15,
        inpaint_freeze_ratio: float = 0.25,
        inpaint_random_freeze_prob: float = 0.25,
    ):
        self.model = model
    ...
    def compute_per_token_scores(self, prefix_tokens: torch.Tensor) -> torch.Tensor:
        """Compute per-token contribution to the task score (for inpainting)."""
        scores = torch.zeros(len(prefix_tokens), device=self.device)
        base_score = self.compute_score(prefix_tokens)

        # Ablation: measure importance of each token by removing it
        with torch.no_grad():
            for i in range(len(prefix_tokens)):
                # Replace token with a neutral token (space or common token)
                modified = prefix_tokens.clone()
                # Use a "neutral" token - the most common token (space)
                space_token = self.tokenizer.encode(" ", add_special_tokens=False)[0]
                modified[i] = space_token
                modified_score = self.compute_score(modified)
                # Higher score drop = more important token
                scores[i] = base_score - modified_score

        return scores

    def compute_cross_entropy(self, prefix_tokens: torch.Tensor) -> float:
        """Compute cross-entropy of the prefix (fluency measure)."""
        with torch.no_grad():
    ...
    def llm_assist(self, population: List[Tuple[torch.Tensor, float, float]]) -> List[torch.Tensor]:
        """
        EPO-Assist: Use the model to generate variations of best candidates.

        From ContextBench: "We periodically provide a SOTA model with the current output
        of EPO and ask it to generate similar inputs."
        """
        variations = []

        # Get the best candidates by score
        sorted_pop = sorted(population, key=lambda x: x[1], reverse=True)
        top_candidates = sorted_pop[:3]  # Take top 3

        # Create a prompt asking for variations
        examples = []
        for tokens, score, xe in top_candidates:
            text = self.tokenizer.decode(tokens)
            examples.append(f"- \"{text}\"")

        prompt = f"""These text prefixes help a model predict the next number in a sequence.
Given prefixes (ranked by effectiveness):
{chr(10).join(examples)}

Generate 3 similar but different prefixes that might also help with number sequence prediction.
Focus on instruction-like text about predicting the next number.
Output only the prefixes, one per line, each on its own line:"""
        ...
        try:
            with torch.no_grad():
                ...
                # Parse generated text into individual prefixes
                ...
                for line in lines[:3]:
                    ...
                    line = line.strip('- "\'')
                    if len(line) > 3:
                        # Tokenize and pad/truncate to prefix_length
            ...
            print(f"  LLM assist failed: {e}")

        return variations

    def inpaint(self, prefix_tokens: torch.Tensor) -> torch.Tensor:
        """
        EPO-Inpainting: Freeze high-activation tokens and resample the rest.

        From ContextBench: "We identify the tokens with maximum activation, freeze them,
        and use a bidirectional language model (LLaDa) to inpaint the intervening tokens."

        Since LLaDA may not be available, we use the base model to resample tokens.
        """
        # Compute per-token importance scores
        scores = self.compute_per_token_scores(prefix_tokens)

        # Determine which tokens to freeze (top 25% by importance)
        num_freeze = max(1, int(self.prefix_length * self.inpaint_freeze_ratio))
        _, top_indices = torch.topk(scores, num_freeze)

        # Create freeze mask
        freeze_mask = torch.zeros(self.prefix_length, dtype=torch.bool, device=self.device)
        freeze_mask[top_indices] = True

        # Also randomly freeze some other tokens (25% probability)
        for i in range(self.prefix_length):
            if not freeze_mask[i] and np.random.random() < self.inpaint_random_freeze_prob:
                freeze_mask[i] = True

        # Resample unfrozen tokens using the model's distribution
        new_tokens = prefix_tokens.clone()

        with torch.no_grad():
            for i in range(self.prefix_length):
                if not freeze_mask[i]:
                    # Use context to predict this token
                    if i == 0:
                        # Sample from unconditional distribution
                        logits = self.model.get_input_embeddings().weight.mean(dim=0)
                        new_tokens[i] = torch.multinomial(F.softmax(logits / 0.8, dim=-1), 1).item()
                    else:
                        # Use prefix up to this point
    ...
    def run(self, num_iterations: int = 150, verbose: bool = True) -> dict:
        """
        Run improved EPO optimization.

        Returns dict with results for each lambda value.
        """
        results = {}
        ...
        for lambda_val in self.lambda_values:
            if verbose:
                print(f"\n{'='*60}")
                print(f"EPO with lambda={lambda_val}")
                print(f"{'='*60}")
            ...
            for iteration in range(num_iterations):
                ...
                for tokens, _, _ in population:
                    ...
                    # Select best candidate for this member
                    best_candidate = max(candidates, key=lambda x: x[1] - lambda_val * x[2])
                    new_population.append(best_candidate)

                population = new_population

                # EPO-Inpainting: every inpaint_interval iterations
                if (iteration + 1) % self.inpaint_interval == 0 and iteration < num_iterations - 10:
                    if verbose:
                        print(f"  Iter {iteration+1}: Running inpainting...")

                    inpainted_population = []
                    for tokens, score, xe in population:
                        inpainted = self.inpaint(tokens)
                        new_score = self.compute_score(inpainted)
                        new_xe = self.compute_cross_entropy(inpainted)

                        # Keep inpainted if it's better or similar
                        if new_score - lambda_val * new_xe >= score - lambda_val * xe - 0.1:
                            inpainted_population.append((inpainted, new_score, new_xe))
                        else:
                            inpainted_population.append((tokens, score, xe))

                    population = inpainted_population

                # EPO-Assist: every assist_interval iterations
                if (iteration + 1) % self.assist_interval == 0 and iteration < num_iterations - 20:
                    if verbose:
                        print(f"  Iter {iteration+1}: Running LLM assist...")

                    variations = self.llm_assist(population)

                    # Add variations to population (replace worst members)
                    for var_tokens in variations:
                        score = self.compute_score(var_tokens)
                        xe = self.compute_cross_entropy(var_tokens)

                        # Replace worst member if variation is better
                ...
                if verbose and (iteration + 1) % 25 == 0:
                    ...
                    print(f"  Iter {iteration+1}: best_score={best_score:.3f}, best_xe={best_xe:.2f}")
            ...
            if verbose:
                print(f"\nFinal (lambda={lambda_val}):")
                print(f"  Score: {best_score:.3f}")
                print(f"  Cross-entropy: {best_xe:.2f}")
                print(f"  Prefix: '{results[lambda_val]['text']}'")

        return results


def evaluate_model(model, tokenizer, device, prefix: str, verbose: bool = False) -> Tuple[float, List[dict]]:
...
def main():
    ...
    # Run improved EPO
    print("\n" + "="*70)
    print("RUNNING IMPROVED EPO (with Assist + Inpainting)")
    print("="*70)
    ...
    results = epo.run(num_iterations=150, verbose=True)

    # Evaluate results
    print("\n" + "="*70)
    print("EVALUATION OF IMPROVED EPO PREFIXES")
    print("="*70)
    ...
    for lambda_val, data in results.items():
        ...
        print(f"\nLambda={lambda_val}: {acc*100:.1f}% accuracy")
        print(f"  Prefix: '{prefix}'")
        print(f"  Score: {data['score']:.3f}, XE: {data['cross_entropy']:.2f}")
    ...
    # Compare with handcrafted prefixes
    print("\n" + "="*70)
    print("COMPARISON WITH HANDCRAFTED PREFIXES")
    print("="*70)
    ...
    for name, data in all_results["handcrafted"].items():
        ...
        colors.append('#2ecc71' if data["accuracy"] >= 0.9 else '#f39c12' if data["accuracy"] >= 0.5 else '#e74c3c')

    # Add improved EPO results
    ...
    ax.set_title('Integer Sequence Prediction: Improved EPO Comparison', fontsize=14)
    ax.set_xlim(0, 105)
